...
# The harness ships inside the standalone exsh repo at tests/. Everything it
# needs is resolved relative to this file so the suite is relocatable:
#   <repo>/tests/exsh_test_harness.py   (this file)        -> HARNESS_ROOT
#   <repo>/tests/tests/manifest.json    (manifest + .exsh)
#   <repo>/tests/fixtures/...           (fixtures)
#   <repo>/examples/...                 (example programs)
#   <repo>/build/exsh                   (default binary)
...
def resolve_script_path(raw_script: str) -> Path:
    """Map a manifest 'script' entry onto the standalone repo layout.

    Manifests are vendored from the umbrella, where scripts are addressed
    repo-relative as ``Tests/exsh/...`` and ``Examples/exsh/...``. In the
    standalone repo those subtrees live at ``tests/...`` and ``examples/...``,
    so translate the legacy prefixes and resolve against this harness's own
    location. Already-relocated or absolute paths are honoured as-is.
    """
    script = raw_script.replace("\\", "/")
    p = Path(script)
    if p.is_absolute():
        return p.resolve()
    if script.startswith("Tests/exsh/"):
        return (HARNESS_ROOT / script[len("Tests/exsh/"):]).resolve()
    if script.startswith("Examples/"):
        return (REPO_ROOT / "examples" / script[len("Examples/"):]).resolve()
    # Fallback: treat as relative to the repo root.
...
def load_manifest(path: Path) -> List[TestCase]:
    ...
    for raw in data.get("tests", []):
        script_path = resolve_script_path(raw["script"])
        tests.append(
            TestCase(
                test_id=raw["id"],
...
def ensure_executable(path_override: Optional[str]) -> Path:
    # Precedence: explicit --bin/--executable, then $EXSH_BIN, then the
    # standalone build output (<repo>/build/exsh).
    if path_override:
        exe = Path(path_override).expanduser()
    elif os.environ.get("EXSH_BIN"):
        exe = Path(os.environ["EXSH_BIN"]).expanduser()
    else:
        exe = REPO_ROOT / "build" / "exsh"
    if not exe.exists():
        raise FileNotFoundError(
            f"exsh executable not found at {exe}; build the project first, "
            f"set EXSH_BIN, or pass --bin"
        )
    return exe
...
def run_exsh(executable: Path, case: TestCase, extra_args: Optional[List[str]] = None) -> subprocess.CompletedProcess[str]:
    ...
    env = os.environ.copy()
    env.update(case.env)
    # Ensure exsh tests find sibling tool binaries built from this workspace.
    # In the standalone build they sit beside the exsh binary (<repo>/build);
    # in the umbrella they share build/bin with exsh. Putting the executable's
    # own directory on PATH covers both layouts.
    tool_bin = str(executable.resolve().parent)
...
def parse_args(argv: List[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--bin", "--executable", dest="executable", type=str, default=None,
                        help="Override path to the exsh executable (also via $EXSH_BIN)")
